...
impl Browser {
    ...
    pub fn input(&mut self, bytes: &[u8], received: Instant) -> Input {
        ...
        while !self.bytes.is_empty() {
            if !self.escaped.is_empty() {
                // The prefix already reached the native parser at its normal
                // Escape deadline. Observe its suffix without delaying/replaying
                // those bytes, so a split paste opener stays locally inert.
                let byte = self.bytes.remove(0);
                self.escaped.push(byte);
                match self.escaped.as_slice() {
                    b"\x1b[200~" => {
                        self.paste = true;
                        self.escaped.clear();
                        self.tail = false;
                    ...
                    b"\x1b]" | b"\x1b_" | b"\x1bP" | b"\x1b^" | b"\x1bX" if !self.paste => {
                        self.string = true;
                        self.escaped.clear();
                        self.tail = false;
                    }
                    prefix
                        if b"\x1b[200~".starts_with(prefix) || b"\x1b[201~".starts_with(prefix) => {
                    }
                    _ => {
                        self.escaped.clear();
                        if byte == 27 {
                        ...
                        self.tail = byte != 0 && byte != 27 && !(0x40..=0x7e).contains(&byte);
                    }
                }
                self.suspend();
                out.bytes.push(byte);
    ...
    pub fn timeout(&mut self) -> Vec<u8> {
        ...
        if !paste_marker && self.since.is_some_and(|at| at.elapsed().as_millis() >= 30) {
            ...
            if !self.string
                && !self.bytes.is_empty()
                && (b"\x1b[200~".starts_with(&self.bytes) || b"\x1b[201~".starts_with(&self.bytes))
            {
                self.escaped.clone_from(&self.bytes);
            }
            self.suspend();
            self.since = None;
...
mod tests {
    ...
    fn fresh(browser: &Browser) -> Instant {
        browser
            .displayed
            .as_ref()
            .map_or_else(Instant::now, |d| d.at)
            + std::time::Duration::from_millis(1)
    }
    #[test]
    fn timed_out_paste_delimiters_remain_inert_and_forward_exact_bytes() {
        const START: &[u8] = b"\x1b[200~";
        const END: &[u8] = b"\x1b[201~";
        for start_split in 1..=2 {
            for end_split in 1..=2 {
                let mut browser = ready();
                let mut native = Vec::new();
                let mut serial = 1;
                for (marker, split) in [(START, start_split), (END, end_split)] {
                    let first = browser.input(&marker[..split], fresh(&browser));
                    assert!(first.bytes.is_empty() && first.open.is_none());
                    browser.since = Some(Instant::now() - std::time::Duration::from_millis(31));
                    assert_eq!(browser.timeout(), marker[..split]);
                    native.extend_from_slice(&marker[..split]);
                    for byte in &marker[split..] {
                        serial += 1;
                        // Even a valid remote context cannot turn a fragmented
                        // physical paste into a fresh local shortcut.
                        arm(&mut browser, serial, &context(0));
                        let result = browser.input(&[*byte], fresh(&browser));
                        assert!(result.open.is_none());
                        native.extend(result.bytes);
                    }
                    if marker == START {
                        serial += 1;
                        arm(&mut browser, serial, &context(0));
                        let pasted = browser.input(b"ip\r", fresh(&browser));
                        assert!(
                            pasted.open.is_none(),
                            "start split {start_split}, end split {end_split}"
                        );
                        native.extend(pasted.bytes);
                    }
                }
                assert_eq!(native, [START, b"ip\r", END].concat());
                serial += 1;
                arm(&mut browser, serial, &context(0));
                assert_eq!(
                    browser.input(b"p", fresh(&browser)).open.as_deref(),
                    Some(PR)
                );
            }
        }
    ...
    #[test]
    fn timed_out_escape_keeps_native_latency_and_does_not_authorize_its_suffix() {
        for prefix in [b"\x1b".as_slice(), b"\x1b["] {
            let mut browser = ready();
            assert!(browser.input(prefix, fresh(&browser)).bytes.is_empty());
            browser.since = Some(Instant::now());
            assert!(browser.timeout().is_empty());
            browser.since = Some(Instant::now() - std::time::Duration::from_millis(31));
            assert_eq!(browser.timeout(), prefix);
            arm(&mut browser, 2, &context(0));
            let suffix = browser.input(b"i", fresh(&browser));
            assert!(suffix.open.is_none());
            assert_eq!(suffix.bytes, b"i");
            arm(&mut browser, 3, &context(0));
            assert_eq!(
                browser.input(b"i", fresh(&browser)).open.as_deref(),
                Some(ISSUE)
            );
        }
    }
