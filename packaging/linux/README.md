...
| Nix | [Source packaging draft](../nix/README.md). Current v0.3.5 recipe has offline preparation checks only. Historical native strict-sandbox builds, all 485 tests and exact inventories passed for v0.3.3 plus the declared parser correction. Separately, native Nix-on-Ubuntu core/companion ownership, UI refusal and normal profile removal passed for unpublished v0.3.4 source `2d52985`. Actual 0.3.3 → 0.3.4 profile upgrades, emulated NixOS core runtime and real SSH from the installed macOS companion to the Nix-built core also passed at their recorded scopes. Physical clipboard remains open. |
