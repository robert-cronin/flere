...
fn catalog() -> Vec<Value> {
    [
("get_context","Read compact identity, assignment, latest checkpoint and pending message summaries. Read bodies through inbox; detail=true adds notes, history and activation diagnostics.",schema(json!({"detail":{"type":"boolean"}}),&[])),
...
("inbox","Read bounded pending bodies; follow next_after with after. Acknowledge exact handled IDs. ack_ids alone returns a receipt without reading more bodies; pending reports remaining work.",schema(json!({"ack_ids":{"type":"array","items":{"type":"string"}},"after":{"type":"string"},"limit":{"type":"integer","minimum":1,"maximum":32}}),&[])),
