...
- Preserve shells across UI detach. Supervisor-only startup and background observation never launch a native chat. User UI opening restores all saved open tabs with their order/selection: exact recorded native conversation per tab, shell directory, or editor path. Unknown native IDs use the harness picker, never a latest-session guess or a Flere UUID form. Keep failed tabs for explicit retry; never replay drafts or commands. Retain stopped workspace metadata.
