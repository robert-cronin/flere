...
downloads match the sealed immutable release from source `d28ad26`, published by
...
selects Linux v0.3.13 or the unsigned macOS arm64 v0.3.0 prebuilt from the
...
| [Cargo / crates.io](https://crates.io/crates/flere/0.3.13) | Core source package for Linux/macOS | Published v0.3.13 through Trusted Publishing from the matching GitHub source. Companion excluded |
| [Debian `.deb`](#install-the-debian-package) | Linux x86_64 with glibc 2.39+ | v0.3.13 download published and independently inspected; native Ubuntu lifecycle and missing-Git download checks passed for v0.3.2 |
