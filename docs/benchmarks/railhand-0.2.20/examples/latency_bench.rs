...
fn resources(
    ...
    let railhand = std::iter::once(f.server.id())
    ...
    while start.elapsed() < Duration::from_secs(seconds) {
        ...
        if start.elapsed() >= next {
            let p = ps(&pids)?;
            samples.push(json!({"seconds":start.elapsed().as_secs_f64(),"railhand_rss_mib":railhand.iter().filter_map(|id|p.get(id)).map(|v|v.1).sum::<u64>() as f64/1024.,"shell_rss_mib":shells.iter().filter_map(|id|p.get(id)).map(|v|v.1).sum::<u64>() as f64/1024.}));
    ...
    Ok(
        json!({"scenario":name,"seconds":elapsed,"railhand_processes":railhand.len(),"shells":shells.len(),"railhand_cpu_percent_one_core":cpu(&railhand),"shell_cpu_percent_one_core":cpu(&shells),"samples":samples}),
    )
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.len() != 2 {
        return Err(
            "usage: latency_bench RAILHAND_EXE OUTPUT.json; run --release, use a home-cache output"
                .into(),
        );
    }
    ...
    for (cols, rows) in [(80, 24), (120, 32), (160, 42)] {
        eprintln!("Measuring {cols}x{rows}: direct PTY and completed Railhand frames");
        ...
        cases.push(json!({"cols":cols,"rows":rows,"direct_pty_echo":baseline,"railhand_completed_frame":measured}));
    }
