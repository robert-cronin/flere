...
v0.3.13 or the unsigned macOS arm64 v0.3.0 prebuilt. Each selected manifest is
...
To select Linux v0.3.13 explicitly instead of following the platform default:

```sh
wget -qO- https://raw.githubusercontent.com/robert-cronin/flere/main/scripts/install.sh | sh -s -- https://github.com/robert-cronin/flere/releases/download/v0.3.13/flere-x86_64-unknown-linux-gnu.manifest.json
```
...
Or [download and verify the public v0.3.13 ZIP](../packaging/windows/README.md#install-the-public-windows-zip).
...
The published [flere 0.3.13 crate](https://crates.io/crates/flere/0.3.13) builds the
...
start chats or publish a release. Public v0.3.13 provides Linux x86_64 GNU
