...
v0.3.10 or the unsigned macOS arm64 v0.3.0 prebuilt. Each selected manifest is
...
To select Linux v0.3.10 explicitly instead of following the platform default:

```sh
wget -qO- https://raw.githubusercontent.com/robert-cronin/flere/main/scripts/install.sh | sh -s -- https://github.com/robert-cronin/flere/releases/download/v0.3.10/flere-x86_64-unknown-linux-gnu.manifest.json
```

This selects the matching core/companion pair; append `--core-only` for a server.
The separate macOS arm64 prebuilt remains unsigned v0.3.0.

## Install on Windows

With [Scoop](https://scoop.sh/) installed, use Flere's published bucket:
...
Or [download and verify the public v0.3.10 ZIP](../packaging/windows/README.md#install-the-public-windows-zip).
...
The published [flere 0.3.10 crate](https://crates.io/crates/flere/0.3.10) builds the
...
```sh
cargo install flere --locked --version 0.3.10
flere --version
```
...
start chats or publish a release. Public v0.3.10 provides Linux x86_64 GNU
...
or an explicit verified package source.
