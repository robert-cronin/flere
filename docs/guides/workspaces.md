...
Use **Ctrl+Space, G** for **Add project**. Select an existing Git directory and optionally name it. Flere finds the main checkout even when you select a subdirectory or linked worktree, then creates or reuses its primary card. The card is marked **main checkout**. Primary means directory ownership only: it has the same agent tools and ordinary pinning as every other card.
